...
// Each exercise: { id, category, title, difficulty, xp, description, tests, hints, starterCode }
...
function ex(id, cat, title, diff, desc, tests, hints = [], starter = '') {
  return { id, category: cat, title, difficulty: diff, xp: XP[diff], description: desc, tests, hints, starterCode: starter };
}
...
`Ask the user for their name with <code>input("Enter your name: ")</code>. Then print <code>Hello, &lt;name&gt;!</code>`,
[t(['Alice'],['Enter your name: ','Hello, Alice!'])],
...
[t(['James','Bond'],['James','Bond'],['James Bond']),
 t(['Emma','Watson'],['Emma','Watson'],['Emma Watson'])],
...
['input() always returns a string — but here you can keep it as string', 'Use + to build the output string, or an f-string']),

ex('var-13','variables','f-String Greeting','medium',
`Ask for the user's name and their favourite subject. Use an <strong>f-string</strong> to print: <code>Hi &lt;name&gt;, I love &lt;subject&gt; too!</code>`,
[t(['Sophie','Maths'],['Hi Sophie, I love Maths too!']),
 t(['Leo','Science'],['Hi Leo, I love Science too!'])],
['f-strings: f"Hello {variable}"', 'Two input() calls needed'],
'name = input()\nsubject = input()\n'),
...
`Assign <code>a = 10</code> and <code>b = 20</code>. Swap their values (without using a third variable or Python\'s tuple swap). Print <code>a</code> then <code>b</code> — expected output: <code>20</code> then <code>10</code>.`,
[t([],['20','10'])],
['Python tuple swap: a, b = b, a', 'This swaps without a temp variable'],
'a = 10\nb = 20\n'),
...
['Build the string with + and ", "', 'Or use f-string with all three'],
'colour = input()\nfood = input()\nsport = input()\n'),
...
['Convert quantity with int(), price with float()', 'Multiply to get total', 'Use f-strings or concatenation for each line'],
'item = input()\nquantity = int(input())\nprice = float(input())\n'),
...
['Four input() calls', 'Build the sentence with an f-string or concatenation'],
'noun = input()\nverb = input()\nadjective = input()\nnumber = input()\n'),
...
['a, b = 0, 1', 'for _ in range(n): print(a); a, b = b, a + b'],
'n = int(input())\n'),
...
['if word in my_list: print(f"Found at index {my_list.index(word)}")', 'else: print("Not found")']),
...
`Define a function <code>add(a, b)</code> that <strong>returns</strong> the sum of a and b. Print the result of calling it with 3 and 7.`,
[t([],['10'])],
...
['for c in range(start, end+1, step):', '    f = round(c * 9/5 + 32, 1)', '    print(f"{c}°C = {f}°F")']),
...
['Use formatted strings: f"{val:4}" right-aligns in 4 chars', 'First row: spaces + "   1   2..."']),
